...
pub fn (mut b Bitstream) more_rbsp_data() bool {
  ...
  */
	bk_index := b.p
	bk_bits_left := b.bits_left
	defer {
		b.p = bk_index
		b.bits_left = bk_bits_left
	}
...
pub fn (mut sh SliceHeader) read_slice_header(nal &NetworkAbstractionLayerHeader, pps_array []PictureParameterSet, sps_array []SequenceParameterSet, mut b Bitstream) {
	...
	if sh.pic_parameter_set_id >= u32(pps_array.len) {
		panic('H.264 slice refers to unavailable PPS ${sh.pic_parameter_set_id}')
	}
	pps := pps_array[sh.pic_parameter_set_id]
	if pps.seq_parameter_set_id >= u32(sps_array.len) {
		panic('H.264 PPS ${sh.pic_parameter_set_id} refers to unavailable SPS ${pps.seq_parameter_set_id}')
	}
	...
	if sh.is_slice_type(SH_SLICE_TYPE.p) || sh.is_slice_type(SH_SLICE_TYPE.sp)
	...
	if pps.entropy_coding_mode_flag != 0 && !sh.is_slice_type(SH_SLICE_TYPE.i)
		&& !sh.is_slice_type(SH_SLICE_TYPE.si) {
